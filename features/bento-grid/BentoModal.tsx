...
export default function BentoModal({ isOpen, onClose, category, pastImages, futureImages }: BentoModalProps) {
    ...
    const images = (() => {
        if (category === "past") {
            if (pastImages?.length) {
                return pastImages.map((src, index) => ({
                    src,
                    alt: `Past project image ${index + 1}`,
                }));
            }

            return DEFAULT_PAST_IMAGES;
        }
    ...
    const nextImage = () => setCurrentIndex((previous) => (previous + 1) % images.length);
    const prevImage = () => setCurrentIndex((previous) => (previous - 1 + images.length) % images.length);
                        ...
                        <div className="absolute bottom-6 inset-x-0 flex justify-center gap-2 z-40">
                            {images.map((_, index) => (
                                <button
                                    ...
                                    className={`h-1 transition-all duration-300 rounded-full ${currentIndex === index ? "w-8 bg-fibi-purple" : "w-2 bg-white/20"}`}
                                />
                            ))}
                        </div>
                    </div>
